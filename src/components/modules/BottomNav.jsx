import React from 'react';
import { useStore } from '../../store/useStore';
import { LayoutDashboard, User, Search, Compass, Brain, Lock } from 'lucide-react';

const BottomNav = () => {
    const { currentStep, setStep } = useStore();

    // Only show on Home (6) and above
    // 6: Home, 7: Profile, 8: Matches, 9: DeepDive, 10: TestHub
    if (currentStep < 6) return null;

    return (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-6 py-3 flex justify-around items-center z-50 pb-safe">
            <button
                onClick={() => setStep(6)}
                className={`flex flex-col items-center space-y-1 transition-colors ${currentStep === 6 ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}
            >
                <LayoutDashboard size={24} />
                <span className="text-[10px] font-medium uppercase tracking-wide">Home</span>
            </button>

            <button
                onClick={() => setStep(8)}
                className={`flex flex-col items-center space-y-1 transition-colors ${currentStep === 8 ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}
            >
                <Search size={24} />
                <span className="text-[10px] font-medium uppercase tracking-wide">Matches</span>
            </button>

            <button
                onClick={() => setStep(10)}
                className={`flex flex-col items-center space-y-1 transition-colors relative ${currentStep === 10 ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}
            >
                <div className="relative">
                    <Brain size={24} />
                    {/* Lock icon removed */}
                </div>
                <span className="text-[10px] font-medium uppercase tracking-wide">Test</span>
            </button>

            <button
                onClick={() => setStep(7)}
                className={`flex flex-col items-center space-y-1 transition-colors ${currentStep === 7 ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}
            >
                <User size={24} />
                <span className="text-[10px] font-medium uppercase tracking-wide">Profiel</span>
            </button>
        </div>
    );
};

export default BottomNav;
